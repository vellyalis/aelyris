...
3. **【戦略・高工数】音声で“指揮”**（BridgeVoice 相当）。「人間が喋る→指揮役が分解→艦隊が
   ビルド」はビジョン(人間→指揮→Runtime)直結の差別化。on-device Whisper 統合は重い。要判断。
...
## 結論
製品体験（テンプレ/テーマ/音声/配布）は BridgeSpace が先行、**自律の深さと API-first
（外部AIが指揮）は Aether が上**。最小コストで体験差を詰めるなら **(1) Fleet含むテンプレート →
(2) テーマプリセット**。差別化を伸ばすなら **(3) 音声指揮**。
