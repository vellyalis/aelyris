...
set/transition 呼び出し
   ...
   ▼
Manager（Mutexロック取得）
   │  1. in-memory を変更（既存ロジック、戻り値 DecisionChange/Result そのまま）
   │  2. 変更があった時だけ Repository.persist(diff) を呼ぶ
   │  3. persist 失敗は Result で上位へ（NFR-1: silent swallow 禁止）
   ▼
...
> **P1で確定した書き込み粒度**: ContextStore は key 単位の差分 upsert/delete でよい。**TaskGraph は「フルスナップショット保存」を採用**。理由=`TaskManager::with_graph_mut` が autonomy ループにグラフを不透明にミューテートさせる逃げ道で、ここで status・crash/rework/timeout カウンタ・branch binding が一括変更される。差分追跡だと「書き漏らしサイト＝silent durability hole」が生まれるため、各ミューテーション後に**全タスクを1トランザクションで再 upsert**（`TaskRepo::save_graph`）。数十タスク規模なので安価で、バグクラスを構造的に消す。no-op の `recompute_ready()` は書かない。
...
- 永続化失敗の扱い（P1で確定）: Manager の `set/transition/with_graph_mut` の戻り値型は**変えない**（`Option<DecisionChange>` / `Result<Vec<String>, TaskGraphError>`）。36+ の呼び出し元へ `Result` を波及させる負債を避け、**persist 失敗は `tracing::error!` で loud にログ**（黙って握り潰さない）。in-memory 変更は走行中セッションの権威として有効。busy_timeout 設定で persist 失敗自体が稀になる（writer 競合は待つ）。将来 UI 警告が要れば Manager に失敗カウンタを足す（契約は壊さない）。
- 起動時 `load_all` 失敗 → デスクトップアプリを丸ごと止めるのは過剰なので、**loud な error ログ＋in-memory継続**（黙ってデータ無視はしない）。P1実装は `lib.rs` setup でこの方針。

## 7. Event Bus pub/sub（P3 設計・先出し）

- 現状 `EventLog`(cap=256, recent/by_channel)。これを **`agent_events` durable log + 単調 `seq` カーソル** に置換。
- 購読者は `since(seq)` で欠番なく取得（ポーリングのままでも**取りこぼさない**＝No-loss達成）。真のpush(tokio broadcast)はオプションでP3後段。
- in-memory ring は「直近表示用キャッシュ」に降格（真実はSQLite）。
