...
- **SEC-1 [HIGH path-traversal] `session_checkpoint` の任意ファイル読取**: fixed with regression tests in this branch. Backend-built handoff paths are sanitized through `summary_files(worktree_path, logical_session_id, seq)`, parent-directory components are rejected, summary/done paths are canonicalized, and containment under `.aelyris/handoff/` is enforced before checkpoint summary reads. Evidence: `src-tauri/src/agent/session_lifecycle.rs` `canonical_summary_files_for_checkpoint` / `reject_parent_dir_components` / `ensure_under_handoff_dir`, plus tests `checkpoint_summary_files_are_backend_built_and_canonical`, `checkpoint_summary_files_reject_parent_dir_components`, `checkpoint_summary_files_sanitize_parent_dir_session_id`, and `checkpoint_summary_files_require_done_marker`.
