...
作成: 2026-06-30 ／ 改訂: 2026-06-30 (rev2, spec rev2 と整合)
...
- **CX-4 [codex review P2] stale 承認の書込防止**: `resolve_interactive_approval`(`src-tauri/src/ipc/send_keys_commands.rs:151`) は `terminal_id` だけを信じて `1`/Esc を書く＝行が stale（人間が既にメニュー回答/status 遅延）だと誤タイミングで PTY に書く。→ **書込前に対象 session が今も `waiting_approval` かつ期待プロンプトであることを検証**（prompt fingerprint 照合）。承認硬化（PR#4 merged コードの follow-up 兼 WU-RT-1）。
...
- Goal: 実 Claude バイトで (a) 許可メニュー構造（カーソル glyph・option 番号・数字キー確定）と (b)「% context left until auto-compact」行フォーマットを採取し fixture 化。
- 手順: native backend か packaged build で `AELYRIS_ENABLE_WEBVIEW2_CDP=1 pnpm tauri:dev` → CDP 9222 → `spawn_interactive_agent`(Bash 誘発 prompt) → `term_snapshot`（grid）／`capture_pane(stripAnsiCodes:true)`／`list_interactive_agents` で採取。token 消費＝オーナー consent 必須。⚠ sidecar dev は monitor へ feed されるが計測/検出の最終確定は実機（[project_surface2_plumbing_done] 制約）。
- 成果物: `src-tauri/src/agent/__fixtures__/` 等に実バイト fixture。RT-1a/PRE-2 の gate はこの fixture で確定。
- Out-of-scope: 実装本体。
...
- Contract: confidence ラベル必須。Codex/Gemini は時間/ターン fallback・unknown。
- Reuse: `detect_permission_menu`/`MENU_BUFFER`（構造検出前例）、`TelemetryConfidence`、`getMaxTokens`。
- Verifier: `scripts/verify-context-proxy.mjs`＋Rust 単体（RT-1a0 fixture で残量・閾値・confidence）。
- 受入: fixture から context% 抽出＋warn/hard 発火、非 Claude は fallback。
...
- **実機スモーク（RT-1a0/PRE-2 必須）**: §RT-1a0 手順で実 Claude のメニュー構造・残量行を採取し検出器確定。token consent 必須。
