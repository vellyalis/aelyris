...
| Release readiness must include the aggregate readiness gate and cannot pass while tmux/shared-workspace/native-terminal claims are blocked. | Release hardening audit and score model | G6 aggregate gate plus release-score integration | `pnpm verify:quality-score`, `.codex-auto/quality/release-quality-score.json` | BLOCK. Current generated artifact is `71/100`, `249/351`, grade `D`, `releaseCandidateReady=false`; regenerate with `pnpm verify:quality-score`; never trust stale prose numbers. |
...
`externalBlockedCount=27`. Safe proof registry coverage is `28/28`. Regenerate
...
5. Release:
   current generated score is `71/100` (`249/351`), grade `D`,
