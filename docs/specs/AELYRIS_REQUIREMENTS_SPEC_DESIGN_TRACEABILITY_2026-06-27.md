...
| Release readiness must include the aggregate readiness gate and cannot pass while tmux/shared-workspace/native-terminal claims are blocked. | Release hardening audit and score model | G6 aggregate gate plus release-score integration | `pnpm verify:quality-score`, `.codex-auto/quality/release-quality-score.json` | Read the regenerated artifact; final-goal audit is downstream and cannot feed points back into the score. Fixed prose scores are not current evidence. |
...
`.codex-auto/quality/final-goal-audit.json` for the current status and
residual-risk classification. Regenerate `pnpm verify:goal:safe` before a
current aggregate claim; do not reuse older external-only safe gate prose. The
...
5. Release:
   read the current score, grade, and release-candidate state from
   `.codex-auto/quality/release-quality-score.json` after regenerating
   `pnpm verify:quality-score`; final-goal audit is downstream and cannot add
   points back into this score. The aggregate readiness gate remains a
   release-score blocker until all claims pass.
