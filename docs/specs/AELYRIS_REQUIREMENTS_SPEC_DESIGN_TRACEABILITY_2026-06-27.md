...
| Release readiness must include the aggregate readiness gate and cannot pass while tmux/shared-workspace/native-terminal claims are blocked. | Release hardening audit and score model | G6 aggregate gate plus release-score integration | `pnpm verify:quality-score`, `.codex-auto/quality/release-quality-score.json` | BLOCK. Current generated artifact is `19/100`, `62/327`, grade `D`, `releaseCandidateReady=false`; final-goal audit is downstream and cannot feed points back into the score; regenerate with `pnpm verify:quality-score`; never trust stale prose numbers. |
...
`externalBlockedCount=15`. `pnpm verify:goal:safe` is currently `blocked`; do
...
5. Release:
   current generated score is `19/100` (`62/327`), grade `D`,
