...
| Release readiness must include the aggregate readiness gate and cannot pass while tmux/shared-workspace/native-terminal claims are blocked. | Release hardening audit and score model | G6 aggregate gate plus release-score integration | `pnpm verify:quality-score`, `.codex-auto/quality/release-quality-score.json` | BLOCK. Current score is `35/100`, `124/351`, grade `D`, `releaseCandidateReady=false`. |
