...
| Release readiness must include the aggregate readiness gate and cannot pass while tmux/shared-workspace/native-terminal claims are blocked. | Release hardening audit and score model | G6 aggregate gate plus release-score integration | `pnpm verify:quality-score`, `.codex-auto/quality/release-quality-score.json` | BLOCK. Current generated artifact is `23/100`, `76/327`, grade `D`, `releaseCandidateReady=false`; final-goal audit is downstream and cannot feed points back into the score; regenerate with `pnpm verify:quality-score`; never trust stale prose numbers. |
...
The final audit status is `blocked` in
`.codex-auto/quality/final-goal-audit.json`, with
`implementationFixableCount=194`, `policyBlockedCount=12`, and
`externalBlockedCount=17`. `pnpm verify:goal:safe` is currently `blocked`; do
not reuse older external-only safe gate prose without rerunning it. Regenerate
...
The safe proof registry target is `28/28`.
...
5. Release:
   current generated score is `23/100` (`76/327`), grade `D`,
   `releaseCandidateReady=false`; final-goal audit is downstream and cannot add
   points back into this score; read the current score and grade from
