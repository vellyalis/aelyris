...
### 18.1 Structured Runtime Candidate Research Contract

`V1-R0` keeps visible PTY as the Current Best and evaluates only structured-runtime
candidates that first pass a bounded admission review. It does not require
OpenCode, ACP, HTTP/SSE, or any other named runtime, and it makes no candidate a
product dependency, runtime owner, public capability, or R0-A9 completion
criterion. Normal execution starts after A9; any earlier work is an independently
authorized, isolated, non-shipping experiment and cannot change the active
remediation order or shipping path.
...
| ID | Contract |
| --- | --- |
| `SR-R0-01` | Freeze the visible PTY baseline, then execute one fixed Mission fixture through PTY and each admitted candidate transport with the same repository, acceptance clauses, provider/model class, and budget class. Missing candidates or transport capabilities are typed `unsupported`, never silently substituted. |
| `SR-R0-02` | Before adapter implementation, record the candidate's structured facts, expected material advantage, artifact/version, source/provenance, license, auto-update state, protocol/schema digest, compatible adapter range, and retirement path. Schema drift fails closed before effect. |
| `SR-R0-03` | Prove the transport-specific isolation boundary: loopback/race-safe port ownership and short-lived credentials for servers, process-tree termination for subprocesses, and secret-free arguments/logs/artifacts for every candidate. |
| `SR-R0-04` | Inventory every candidate config and credential source, including global, project, custom, inline, managed, environment, `.env`, and provider-auth paths. Prove an isolated effective configuration. If OpenCode is voluntarily admitted, `OPENCODE_CONFIG_DIR` alone remains insufficient because sources are merged. |
| `SR-R0-05` | Map external session, tool, diff, permission, status, usage, and error facts into existing `AgentSession`, WorkExecutionAttempt/generation, WorkEvent/Chronicle, capability, Proofbook, review, and merge owners. No second runtime graph, journal, permission authority, completion truth, or merge owner is allowed. |
| `SR-R0-06` | Route every effect through the canonical Control Kernel and Aelyris capability lease. A candidate-native allow/deny prompt is adapter evidence and UI, not authority to grant or widen capability. |
| `SR-R0-07` | Prove abort, disconnect, reconnect, duplicate/out-of-order event, process crash, restart, fork/resume, uncertain effect, and reconciliation behavior. Delivery is at-least-once with idempotent consumers; exactly-once is not claimed. |
| `SR-R0-08` | Measure capability coverage, structured-event fidelity, deny equivalence, diff/evidence completeness, reconnect loss, operator visibility, latency, cost, and secret leakage against the visible PTY Current Best. |
| `SR-R0-09` | Preserve Mission/runtime/evidence identity across adapter disable or retirement and prove visible PTY fallback without rewriting accepted history. |
| `SR-R0-10` | Keep an Aelyris Runtime TUI outside this comparison. It receives a separate product-surface decision only after a structured adapter and daemon-owned projection are proven. |

The decision artifact returns `promote_none`, `promote_one`, `hold`, or `reject`:

- `promote_none` records that no candidate cleared admission or materially beat
  PTY. It is a successful V1-R0 outcome, retains visible PTY, and creates no
  production adapter or downstream V1 obligation;

- `promote_one` names exactly one candidate transport, demonstrates a material Goal
...
Candidate documentation and schemas are discovery inputs. When OpenCode is
voluntarily admitted, the upstream
[server contract](https://opencode.ai/docs/server/),
[ACP contract](https://opencode.ai/docs/acp/), and
[configuration precedence](https://opencode.ai/docs/config/) may be used under the
same candidate-neutral rules. Executed evidence from the pinned artifact remains
authoritative. The V1-R0 packet must trace every `SR-R0-*` requirement to fixed
positive and negative fixtures and emit a machine-readable decision artifact.
...
structured-runtime adapter/Runtime TUI, and a new assurance score. Existing V5 typed deliberation
...
| `verify:structured-runtime-candidate` | admitted-candidate versus PTY comparison and `promote_none`/`promote_one`/`hold`/`reject` decision | unqualified candidate, missing binary, schema drift, merged-config or credential leak, permission bypass, duplicate owner, uncertain replay, fallback loss |
...
| Structured-runtime candidate adapter | `BS-11` + `EV-01` | §18.1 plus existing runtime/control/evidence owners | none; V1-R0 cannot receive R0-A9 credit | `verify:structured-runtime-candidate` |
