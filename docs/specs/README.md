...
| [../../product-delivery-instructions.md](../../product-delivery-instructions.md) | **active product-delivery work order / GMV-3** | GMV-0/1でcockpit goal→visible work→exact candidate review→OID-bound mergeを実機完走し、GMV-2でGoal・planner identity・TaskGraphのatomic durabilityと再起動復元を実証。次は既存settlement/completion ownerを同じ導線へ接続する |
...
root `product-delivery-instructions.md` のactive `GMV-3`であり、general Mission verticalが
