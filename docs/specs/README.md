...
| [PROOFBOOK_AUTOMATION_SPEC.md](./PROOFBOOK_AUTOMATION_SPEC.md) | proposal / automation roadmap + PB-2 local backend runner/ledger + PB-3 MCP integration slice | **Proofbooks**。Scape Playbooks 型の多段 automation を Aelyris の verifier / evidence artifact / governance / visible fleet / merge gate に接続する設計。PB-2 local backend runner/ledger は `shell`・`verifier`・`waitFor`・`manualGate` を Tauri IPC 経由で扱い、PB-3 MCP integration slice は既存 `tools/call` schema/governance 経路で cataloged run/status verbs と `mcpTool` を扱うが、Proofbook UI / create・update・distill / agent・HTTP・fan-out・subProofbook・Evidence Store は未実装の設計 target であり、Proofbooks 全体の実装済みclaimではない |
