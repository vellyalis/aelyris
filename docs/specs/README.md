...
| [PROOFBOOK_AUTOMATION_SPEC.md](./PROOFBOOK_AUTOMATION_SPEC.md) | proposal / automation roadmap + PB-2 local backend runner/ledger | **Proofbooks**。Scape Playbooks 型の多段 automation を Aelyris の verifier / evidence artifact / governance / visible fleet / merge gate に接続する設計。PB-2 local backend runner/ledger は `shell`・`verifier`・`waitFor`・`manualGate` を Tauri IPC 経由で扱うが、Proofbook UI / MCP verbs / agent・HTTP・fan-out・subProofbook・distill は未実装の設計 target であり、Proofbooks 全体の実装済みclaimではない |
