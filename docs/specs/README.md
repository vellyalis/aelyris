...
current frontier は root work order が所有する A9.3 A4 acceptance timeout budget
closure。A9.0 inventory は `refresh_before_fix`、A9.1 fresh owner split は
`repair_dependency_graph`、A9.2 provider-guard boundary correction は
`reject_direct_descriptor` で完了し、token-bearing guard を no-token graph へ
