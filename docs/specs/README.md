...
current frontier は root work order が所有する A9.2 no-token provider-guard
dependency closure。A9.0 inventory は `refresh_before_fix`、A9.1 fresh owner split
は `repair_dependency_graph` で完了し、stale snapshot と downstream view を直接の
実装欠陥へ昇格していない。A8.0 は N4 direction を承認したが
