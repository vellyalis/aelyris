...
多エージェント並列開発コックピット化と world-class terminal AI OS を目指す
...
Aether を「**単一指示で自律ビルドする** AIワークスペース」に近づける。能力（worktree/agent/pane/diff/task/event/context/merge/approval）を**1つの能力レイヤー（Aether Control API）**に集約し、**2つの顔**が投影する: ① 人間の Cockpit UI（Tauri IPC）② オーケストレーターAI（`aether` MCP server）。Reviewer agent による merge は target / gated design であり、現在の製品完成主張ではない。危険シェル/FS操作の **tool-approval は別軸で watchdog ゲート維持**。
...
| [AETHER_FUSION_COORDINATOR_AUDIT_2026-06-23.md](./AETHER_FUSION_COORDINATOR_AUDIT_2026-06-23.md) | future / composition | **Fusion Coordinator 監査**。LLM重み合体ではなく inference-time multi-agent / MoA / swarm coordination を Aether Control API 上に載せる判断、Claude `-p` 監査結果、API shape、安全境界、実装順序 |
