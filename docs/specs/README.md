...
| [../../product-delivery-instructions.md](../../product-delivery-instructions.md) | **active product-delivery work order / AIO-51** | GMV-0..3とAIO-44..50でMCP Goal→current Mission→visible PTY実装→current-Mission-scoped review→exact-OID settlement→completion receipt→bounded historyを既存ownerへ接続済み。次はそのsafe projectionを既存cockpitへ表示する |
...
root `product-delivery-instructions.md` のactive `AIO-51`であり、GMV-0..3とAIO-44..50を
completed foundationとしてcockpit Mission history/receipt inspectionを閉じるsole repo-mutating
