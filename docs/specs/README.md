...
| [PROOFBOOK_AUTOMATION_SPEC.md](./PROOFBOOK_AUTOMATION_SPEC.md) | proposal / automation roadmap + PB-2 local backend runner/ledger + PB-3D MCP design gate | **Proofbooks**。Scape Playbooks 型の多段 automation を Aelyris の verifier / evidence artifact / governance / visible fleet / merge gate に接続する設計。PB-2 local backend runner/ledger は `shell`・`verifier`・`waitFor`・`manualGate` を Tauri IPC 経由で扱い、PB-3D は MCP integration の詳細設計 gate を固定するが、Proofbook UI / MCP verbs / agent・HTTP・fan-out・subProofbook・distill は未実装の設計 target であり、Proofbooks 全体の実装済みclaimではない |
