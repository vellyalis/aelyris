...
| `aelyris.fleet_status` | **params** `{}` → **return** `{ available: boolean, source: "rust-agent-manager", sessions: FleetSessionProjection[], telemetryBoundary: "reported_aelyris_telemetry", providerBillingClaimed: false, promptValuesExposed: false, workspacePathsExposed: false, readOnly: true }` | `AgentManager::list_sessions` → `control::agent::list_headless` → private MCP fleet projection | FREE | Keeps exact session identity, run status, model, reported usage, timestamps, and bounded coordination flags. It excludes prompt/approval text, cwd/workspace/repository/worktree paths, branch names, full lineage, and recycle failure contents. The projection does not reap, stop, resume, or otherwise mutate a session. |
