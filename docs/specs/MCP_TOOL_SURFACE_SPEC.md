...
|------|-----------|---------|------------|-------|
| `aelyris.worktree.list` | **params** `{ repoPath: string }` → **return** `{ repositoryDigest: string, source: "git-worktree-owner", worktreeCount: integer, worktrees: [{ name, branch, isMain, headSha, status }], repositoryPathExposed: false, worktreePathsExposed: false, readOnly: true }` | `control::worktree::list` / `git::list_worktrees` → private path-minimized inventory projection | FREE | Uses the existing Git owner, returns main first and linked worktrees sorted by exact name, and preserves branch/HEAD/status. The input repository path and every absolute worktree path are excluded; invalid repositories remain typed failures and the read performs no create/prune/lock/mutation. |
...
| `aelyris.fleet_status` | **params** `{}` → **return** `{ available: boolean, headlessAvailable: boolean, interactiveAvailable: boolean, headlessCount: integer, interactiveCount: integer, source: "rust-agent-managers", sessions: FleetSessionProjection[], telemetryBoundary: "reported_aelyris_telemetry", providerBillingClaimed: false, promptValuesExposed: false, workspacePathsExposed: false, readOnly: true }` | `AgentManager::list_sessions` + `InteractiveSessionManager::list` → existing `AgentSession::from` conversions → private MCP fleet projection | FREE | Deterministically merges headless and visible interactive sessions by exact id; a duplicate id fails closed. Keeps run mode, identity, status, model, reported usage, timestamps, and bounded coordination flags. It excludes prompt/approval text, cwd/workspace/repository/worktree paths, branch names, full lineage, and recycle failure contents, and does not mutate either runtime owner. |
| `aelyris.agent.activity` | **params** `{}` → **return** `{ available: boolean, source: "rust-agent-manager", sessionCount: integer, fleet: AgentActivityProjection[], activityTargetValuesExposed: true, promptValuesExposed: false, interactiveActivityInvented: false, readOnly: true }` | `AgentManager::list_sessions` → private deterministic activity projection | FREE | Sorts exact headless session ids and preserves the existing typed `AgentActivity { action, file?, symbol?, updated_at }` coordination values with task/status/model metadata. An unattached owner returns `available:false` and an empty fleet. It does not expose prompts/cwd or invent activity for visible interactive sessions, and performs no mutation. |
| `aelyris.list_pending_approvals` | **params** `{}` → **return** `{ pendingQueueAvailable: true, mergeStoreAvailable: boolean, pendingCount: integer, mergeIntentCount: integer, pending: PendingApprovalProjection[], mergeIntents: MergeIntentProjection[], decisionValuesExposed: false, mergeTargetsExposed: false, grantToolExposed: false, readOnly: true }` | bounded `mcp_pending` queue + durable `MergeIntentStore::list_unresolved` → private minimized projections | GATED_OBSERVE_ONLY | Preserves live queue order and deterministically sorts exact durable intent ids. Approval title/summary/raw session id and merge repository/branch/OID/task/session/reviewer/gate values are excluded; only a one-way approval-session digest and merge evidence-presence flags remain. Missing merge storage is explicit and returns an empty merge list without mutating either owner. |
...
| `aelyris.cost.can_spawn` | **params** `{ active_agents: integer, tokens_used: integer\|null, cost_usd: number\|null, runtime_secs: integer\|null }` → **return** `{ evaluable: boolean, decision: SpawnDecision\|null, enabledAxes: string[], unknownAxes: string[], ignoredUnknownAxes: string[], reportedUsage: object, source: "shared-cost-manager", telemetryBoundary: "reported_aelyris_telemetry", providerBillingClaimed: false, unknownUsageZeroFilled: false, readOnly: true }` | shared `CostManager::can_spawn` | FREE | Calls the one existing admission owner only when every enabled cap axis has reported usage. Unknown usage on an enabled cap returns `evaluable:false` and no decision; disabled axes may remain `null` and are listed separately rather than zero-filled. |
| `aelyris.route_agent` | **params** `{ prompt: string, budgetRemaining?: number }` → **return** `{ decision: RoutingDecision, source: "shared-agent-router", promptEchoed: false, readOnly: true }` | shared `control::agent::route` | FREE | Uses the existing routing heuristics and model-profile representation without reflecting the prompt. The recommendation does not start an agent, reserve budget, or establish provider availability. |
