...
| Root work-order status | `product-delivery-instructions.md` owns active `GMV-3`; GMV-0/1/2 are product-accessible and `audit-remediation-instructions.md` is certification-only after A9.6r1 | one repo-mutating phase at a time; certification-only external work may coexist without repository edits |
