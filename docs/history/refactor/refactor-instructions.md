# docs/history/refactor/refactor-instructions.md

> 渡し方: 実装担当モデル（Codex / Opus 等）に
> 「/goal docs/history/refactor/refactor-instructions.md に書かれたことを完遂しろ」で渡す。
...
Aelyris（Tauri v2 + React 19 + Rust）の技術的負債を、**挙動を一切変えずに**段階的に削減する。スコープを3層に分ける:
...
- `docs/specs/AELYRIS_COCKPIT_REQUIREMENTS_2026-06-13.md` = 要件の権威（BR4-9）。
...
- ネイティブ spike binary: `src-tauri/src/bin/aelyris_native.rs`（8212行・**未出荷の spike**）, 出荷 CLI `src-tauri/src/bin/aelys.rs`（PTY sidecar 制御）。
...
- HTTP/MCP: `src-tauri/src/api/`（loopback Axum + `aelyris.mcp.v1` 51 verb）。
...
自律ループ: `orchestrator::autonomy::step`（純粋 `&mut TaskGraph` 状態機械）を `control::loop_ports::run_step` が graph Mutex 下で駆動。IPC face（`orchestrator_step`）と MCP face（`aelyris.orchestrator.step`）が同一 `run_step`／同一 state を共有。
...
9. **IPC の `analyze_agent_stream_line` は advisory のみ**（`commands.rs:3056`、AutoDeny は log/emit のみで headless 子を止めない）。実enforcement は MCP `aelyris.request_approval`（`mcp.rs:961`）だけ。この非対称を前提に。
10. **merge は構造的にのみ gate**: `aelyris.request_approval`/`request_merge` は queue するだけ、実 git merge は `aelyris.review.approve`（`mcp.rs:1102`）が pending→merging を lock 下で claim してから。MCP `safety` フィールド（FREE/GATED）は**宣言的メタデータで enforcement ではない**（C-18）。
...
32. **`AELYRIS_VITE_NO_ESBUILD_SPAWN=1`** は transform pipeline 全体を切替（env-inline chain）。`vite-windows-net-use-shim.cjs` は Windows EPERM 回避。どちらも load-bearing。
...
- **C-15 🔴 `aelyris_native.rs` 8212行 spike** — 未出荷・native-renderer track 所有・`score-release-quality.mjs` がソース substring を gate。**機械分割不可**。promote/retire まで monolith 維持か module 化かを Q。
...
- **C-21 🔴 ephemeral token を log 出力**（`AELYRIS_API_TOKEN` 未設定時、runtime 全体を `127.0.0.1:9333` に自動 bind し token を log）— log 読める local process が spawn_agent+review.approve 権限取得。fingerprint のみ log / 明示設定要求を Q。
