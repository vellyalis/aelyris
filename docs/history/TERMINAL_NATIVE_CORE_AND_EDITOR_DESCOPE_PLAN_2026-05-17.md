...
- `authenticated-ai-cli-consent-packet` is green and fixes the exact opt-in path: set `QUORUM_AUTH_PROMPT_CONSENT=I_UNDERSTAND_THIS_MAY_SPEND_TOKENS` plus `QUORUM_AUTH_PROMPT_PROVIDER=codex|claude|gemini` before running the authenticated prompt smoke.
