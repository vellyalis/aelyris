...
- `authenticated-ai-cli-consent-packet` is green and fixes the exact opt-in path: set `AELYRIS_AUTH_PROMPT_CONSENT=I_UNDERSTAND_THIS_MAY_SPEND_TOKENS` plus `AELYRIS_AUTH_PROMPT_PROVIDER=codex|claude|gemini` before running the authenticated prompt smoke.
...
4. Grow the `aelyris-native` attaching client into a real terminal window only after each step keeps using the same Rust mux daemon/API.
...
- Keep SCM metadata and review queue in Aelyris.
...
- Opening a file from any Aelyris surface opens VSCode at the file/line.
