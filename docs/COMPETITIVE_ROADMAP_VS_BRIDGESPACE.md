# Competitive Roadmap — Aether を「BridgeSpace + tmux 以上」へ
...
**あなたの土俵で勝つ。** Aether の堀＝**壊れない・監査できる・決定的なAI組織OS** + **best-in-class なターミナル持続性** + **深いエディタ(LSP+Vim+Diff)**。①監査でここは既に BridgeSpace/tmux より深いと実証済み。
...
|----|------|------------|------|---------|
| T1-1 | **Run Timeline / Replay**: 永続Event Bus を時系列ビューに。`aether.event.since(seq)` で全イベントを欠番なく再生(タスク生成→dispatch→review→merge→escalation) | event_repo(P3, no-loss durable) | M | BridgeSpace に**無い「決定的リプレイ」**。「何が起きたか完全再現」 |
...
## 北極星(一言)
**「速くて賑やかな vibe coding」ではなく、「壊れない・見える・監査できるAI組織OS」。** 表層を最小限追いつかせ、既に深い堀(堅牢runtime + 持続ターミナル + 深いエディタ)を可視化して尖らせる。
