...
| Tests | cargo test (171 cases) + vitest (888 cases) |

### 1.2 git state

- **master**: `2804547` "fix(round-12): keep unknown conductor roles visible"
- **branches**: master のみ
...
2. **Tauri 実機 visual verify (累積 69 項目)**: Rust + Frontend 同時再ビルド後に通しで実施。 round 6/7/8/9/10/11/12 累積項目を §6 から拾うこと。
...
## 6. 累積 Tauri 実機 visual verify 項目 (69 項目)
...
3. 累積 69 項目を §6 順に通し verify。 結果を `docs/handoff/v0.2.4-verify-result.md` に記録。
