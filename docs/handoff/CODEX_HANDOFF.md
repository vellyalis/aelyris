...
19d2b36 docs(handoff): record round-11 terminal image fix
...
2. **Tauri 実機 visual verify (累積 73 項目)**: Rust + Frontend 同時再ビルド後に通しで実施。 round 6/7/8/9/10/11/12/13 累積項目を §6 から拾うこと。
...
## 6. 累積 Tauri 実機 visual verify 項目 (73 項目)

dogfood 機 (Win11 25H2 build 26200+) で通し実施する。 内訳は memory `project_next_session_handoff.md` に詳細あり。 圧縮版を以下に再掲:

### round 13 で追加 (4 項目)

70. **AgentTerminal exited IME guard**: interactive agent PTY を exit させた後、 IME bar が disabled 表示になり、 `Ctrl+Shift+J` / Enter submit / クリック focus で `write_terminal` が走らないことを確認。 旧バグでは入力が消えて何も起きない。
71. **DiffViewer glyph callback freshness**: DiffViewer 表示中に親 state を更新してから modified 側 glyph margin をクリックし、 最新の handler で comment / action UI が開くことを確認。 旧バグでは初回 mount 時の stale callback が使われる。
72. **InteractiveSessionCard model-specific context gauge**: `gemini-2.5-pro` 等の大きい context model で `tokens_used=10000` 付近の session card を表示し、 context 使用率が 95% ではなく model 上限に応じた低い値になることを確認。
73. **Nested action control regression**: Worktree delete、 WorkspaceTabs close、 Workflow gate approve/reject をクリックしても、 親 row/tab/step の switch/select/expand が同時発火しないことを確認。

### round 12 で追加 (1 項目)

69. **ConductorView unknown role node retention**: devtools / fixture で `role: "planner"` など未知 role の parent session と、 `handoffFrom` が parent を指す reviewer child を ConductorView に渡す。 parent は Ad-hoc column に表示され、 `parent->child` edge も残ることを確認 (旧バグだと parent node と edge が無言で消える)。
...
3. 累積 73 項目を §6 順に通し verify。 結果を `docs/handoff/v0.2.4-verify-result.md` に記録。
