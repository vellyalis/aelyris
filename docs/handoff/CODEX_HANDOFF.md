...
| Tests | cargo test (171 cases) + vitest (882 cases) |

### 1.2 git state

- **master**: `b0b9362` "fix(round-9): clear hunkAnchors when editor handle goes null"
- **branches**: master のみ
...
2. **Tauri 実機 visual verify (累積 66 項目)**: Rust + Frontend 同時再ビルド後に通しで実施。 round 6/7/8/9 累積項目を §6 から拾うこと。
...
| 21 | `src/__tests__/useTerminalImages.test.tsx` | position-churn test 強化 (pending fetch を hold して storm を直接 exercise) | test strength |
| 22 | `src/features/editor/useGhostPaintForFile.ts` | `acceptAllInFile` が editor=null で `layersRef.current` 経由 IPC 発火し得る | silent corruption (round 9 で deferred) |
...
## 6. 累積 Tauri 実機 visual verify 項目 (66 項目)
...
3. 累積 66 項目を §6 順に通し verify。 結果を `docs/handoff/v0.2.4-verify-result.md` に記録。
