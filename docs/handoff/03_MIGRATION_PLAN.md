...
## Phase 5: 統合テスト + リリース ✅ 完了

**目標**: ネイティブバイナリの安定性確認 + リリース準備

```
Status: ✅ Phase 5 完了
Binary: native-terminal.exe (14 MB release)
Tests: 156 pass
```
...
| 要件 | ネイティブ | 状態 | 備考 |
|------|-----------|------|------|
| MR-1: GPU描画ターミナル | ✅ | wgpu DX12, 60fps, ANSI 256色+TrueColor, スクロールバック10K行, テキスト選択+コピー |
| MR-2: PTY管理 | ⚠️ | PowerShell動作確認済。複数PTY(タブUI有、実体は単一PTY) |
| MR-3: ウィンドウ管理 | ✅ | Mica透過, カスタムタイトルバー, DPI対応(winit) |
| MR-4: キーボード入力 | ✅ | ASCII全文字, 特殊キー, Ctrl修飾, IME(CJK), コピペ |
| MR-5: AIエージェント | 📋 | PTY経由でCLIツール利用可。専用UI未実装 |
| MR-6: Git統合 | ⚠️ | ブランチ表示(ステータスバー)。Diff/Worktree未実装 |
| MR-7: データ永続化 | 📋 | 未実装。Tauri版のSQLite/TOML基盤は既存 |
...
| タスク | 優先度 | 詳細 |
|--------|--------|------|
| 複数PTYタブ | HIGH | タブUIは実装済、PTY切替ロジックが未実装 |
| コマンドパレット | MEDIUM | テキスト入力ウィジェットが必要 |
| ウィンドウ位置記憶 | LOW | TOML設定ファイルと連携 |
| インストーラー | MEDIUM | cargo-wix or NSIS |
| AIエージェントUI | LOW | Tauri版からの移植 |
| Git Diff/Worktree | LOW | git2-rs は既に依存にある |
| SQLite永続化 | LOW | rusqliteは既に依存にある |

## タイムライン (実績)

| Phase | 状態 | 実績 |
|-------|------|------|
| Phase 1: ターミナルコア | ✅ | PTY + Grid + GPU描画 + IME + 選択 + スクロール |
| Phase 2: UI Chrome | ✅ | タイトルバー + タブバー + ステータスバー (egui不採用 → wgpu直描画) |
| Phase 3: サイドバー | ✅ | ファイルツリー + Ctrl+B トグル |
| Phase 4: エディタ | ✅ | ビューア + ropey編集 + tree-sitter + LSP |
| Phase 5: テスト+リリース | ✅ | 156テスト、14MB リリースバイナリ |

## リスク (振り返り)

| リスク | 結果 |
|--------|------|
| egui のカスタマイズ性不足 | 発生 → wgpu直描画で全UI実装。結果的に依存ゼロで良い判断 |
