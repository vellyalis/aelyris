...
Source plan: `docs/AI_WORKSTATION_98_IMPLEMENTATION_PLAN_2026-05-02.md`
...
- `C:\Users\owner\.codex\codex-progress-server.mjs` derives canonical dashboard identity from `.codex-auto/current-dashboard.json`.
...
- `C:\Users\owner\.codex\codex-blocker-taxonomy.mjs` now emits the approved blocker kinds: `permission`, `external_dependency`, `validation_failed`, `oversized_task`, `timeout`, `product_decision`, `environment_down`, `test_flake`, `code_conflict`, `destructive`, and `unknown`.
...
- `C:\Users\owner\.codex\codex-blocker-taxonomy.mjs` now normalizes clear `not_blocked` state to `kind=unknown`, `retryPolicy.action=none`, and `notifyUser=false`, even when older artifacts carried legacy blocker kinds.
- `C:\Users\owner\.codex\codex-longrun-watchdog.mjs` uses `isBlockingAnalysis` so a clear fallback artifact does not suppress recovery or trigger no-restart behavior.
- `C:\Users\owner\.codex\codex-auto-loop.mjs` enforces retry `maxAttempts`; retry-cap exhaustion stops with `needs_attention`.
...
- `C:\Users\owner\.codex\codex-blocker-taxonomy.mjs` now applies probe/backoff behavior to both `external_dependency` and `environment_down`.
...
- `C:\Users\owner\.codex\codex-auto-loop.mjs` and `C:\Users\owner\.codex\codex-longrun-watchdog.mjs` use the shared probe path for both probeable blocker kinds.
- `C:\Users\owner\.codex\codex-longrun-monitor.mjs` emits `probe-backoff` events so probe cadence is visible.
...
- `C:\Users\owner\.codex\codex-longrun-watchdog.mjs` clears `activeRoadmap` and `activeSubtask` when the run is complete.
- `C:\Users\owner\.codex\codex-progress-server.mjs` returns no active card when roadmap/status is complete.
- `C:\Users\owner\.codex\codex-longrun-monitor.mjs` suppresses active-card text for completed summaries.
- `C:\Users\owner\.codex\codex-blocker-taxonomy.mjs` ignores stale blocking fallback analysis from a different active roadmap card.
...
- `C:\Users\owner\.codex\codex-notification-store.mjs` now persists notification-center entries to both `.codex-auto/current-notifications.json` and `.codex-auto/notifications.jsonl` with dashboard/local fallback metadata.
- `C:\Users\owner\.codex\codex-auto-loop.mjs` writes final-report summary data into `current-progress.json` heartbeat fields and completion/attention notifications when the loop stops.
- `C:\Users\owner\.codex\codex-progress-server.mjs` exposes `finalReport` and `notificationStore` in `/state`, renders a dashboard `Final Report` section, and shows an in-dashboard browser notification fallback for denied/unsupported permissions.
- `C:\Users\owner\.codex\codex-longrun-selftest.mjs` now covers final report `/state`/dashboard surfacing and denied notification JSON/JSONL fallback persistence.
...
- `C:\Users\owner\.codex\codex-auto-loop.mjs` now normalizes generated and decomposed work to typed lineage reasons: `original-roadmap-continuation`, `blocker-decomposition`, `improvement-slice`, or `user-requested-new-task`.
...
- `C:\Users\owner\.codex\codex-progress-server.mjs` renders lineage in the decomposition queue, exposes roadmap lineage in `/state`, and adds the `task-lineage-required` dashboard sentinel.
- `C:\Users\owner\.codex\codex-longrun-selftest.mjs` covers missing-lineage rejection, legacy reason normalization, `/state` lineage, and dashboard HTML lineage visibility.
...
| Dashboard scroll/view-state | `node --check C:/Users/owner/.codex/codex-progress-server.mjs` plus temporary Playwright dashboard smoke | Validates dashboard syntax, state-version diff skipping, scroll anchoring, and preserved details/filter/sort/tab/focus state. |
...
- `C:/Users/owner/.codex/codex-auto-loop.mjs` reconciliation now resolves stale done embedded `current-progress.activeRoadmap` objects to the current doing roadmap card, preventing P0-14/P0-15 ghosts after the roadmap advances.
...
- Reason: multiple same-workspace `codex-auto-loop.mjs` processes are concurrently writing `.codex-auto` artifacts for `C:/Users/owner/Aether_Terminal`, causing top-level and embedded active roadmap state to diverge.
...
- Fresh read-only process probe still found six same-workspace `codex-auto-loop.mjs` writers for `C:/Users/owner/Aether_Terminal`: `21032`, `21572`, `27444`, `32984`, `36352`, `1244`.
...
- Docs changed: `docs/AI_WORKSTATION_98_OPERATIONAL_RUNBOOKS.md` and `docs/release-build-playbook.md`.
