      ...
      (cmd) => {
        if (cmd === "list_snapshots")
          return Promise.resolve([
            makeSummary({ id: "a", capturedAt: 100 }),
            makeSummary({ id: "b", capturedAt: 200 }),
          ]);
      ...
      (cmd, args) => {
        ...
        if (cmd === "get_snapshot")
          return Promise.resolve({
            ...
            capturedAt: 0,
            trigger: { kind: "userSubmitted" },
            grid: { cols: 2, rows: 1, cells: [[]], cursor: {
              row: 0, col: 0, shape: "block", blinking: false, visible: true,
            } },
          });
        return Promise.reject(new Error(`unexpected ${cmd}`));
      },
