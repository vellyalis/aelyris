...
describe("NativeTerminalArea initial output replay", () => {
  it("subscribes first, then replays buffered pane output into AI CLI detection", () => {
    const src = getSrc();
    const effect = src.match(/await subscribeOutput\(terminalId[\s\S]*?subscribe_pty_output/);
    expect(effect).not.toBeNull();
    const body = effect?.[0] ?? "";

    expect(body.indexOf("await subscribeOutput(terminalId")).toBeLessThan(
      body.indexOf('invoke<string>("capture_pane"'),
    );
