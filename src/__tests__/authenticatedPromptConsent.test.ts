...
describe("authenticated prompt consent packet", () => {
  it("turns the opt-in smoke artifact into a ready no-token consent packet", () => {
    ...
    expect(packet.requiredEnv).toBe("AETHER_AUTH_PROMPT_CONSENT=I_UNDERSTAND_THIS_MAY_SPEND_TOKENS");
  ...
  it("merges the provider preflight matrix into the consent packet", () => {
    ...
    expect(packet.providerReadiness[1]?.requiredEnv).toContain("AETHER_AUTH_PROMPT_PROVIDER=claude");
