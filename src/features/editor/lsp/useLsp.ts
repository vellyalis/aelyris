...
export function useLsp({ projectPath, monacoLanguage }: UseLspOptions) {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        if (aborted) { invoke("lsp_stop", { language, rootPath: projectPath }).catch(() => {}); return; }
        ...
        const unsub = await listen<{ server: string; message: string }>("lsp:response", (event) => {
          try {
            ...
            if (!initialized.current && resp.result && typeof resp.result === "object" && "capabilities" in (resp.result as Record<string, unknown>)) {
  ...
  const sendRequest = useCallback(async (method: string, params: unknown): Promise<JsonRpcResponse | null> => {
  ...
  const notifyOpen = useCallback((uri: string, languageId: string, content: string) => {
    if (!language) return;
  ...
  const notifyChange = useCallback((uri: string, version: number, content: string) => {
    if (!language) return;
    const notif = JSON.stringify({
      jsonrpc: "2.0",
