...
4. `product-delivery-instructions.md` - **ACTIVE / AIO-48 Mission settlement scope**.
   ...
   submission to the existing planner and atomic Mission/TaskGraph owner; AIO-46
   added value-minimized exact current Mission/plan/TaskGraph recovery after reconnect
   or restart; AIO-47 routes authenticated MCP Mission advancement through the existing
   visible cockpit `orchestrator_step` / PaneFleet owner with backend-derived honest
   admission facts and stops at Review. AIO-48 is the sole repo-mutating frontier:
   require `mission.review_and_settle` to prove exact membership in the current accepted
   Mission before any reviewer, candidate, or merge effect. Do not create a second
   planner, TaskGraph, Mission cache, dispatcher, PaneFleet, reviewer, or merge owner.
...
R0..A9/A9.6r1 (repo complete) -> product delivery GMV-0..3/AIO-47 (complete) -> AIO-48 (active)`。A9 certification は
