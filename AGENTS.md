...
- Current machine truth refreshed 2026-07-10 JST by the comprehensive audit safe
  chain: `pnpm verify:quality-score` = `65/100` (`228/351`), grade `D`,
  `releaseCandidateReady=false`.
  `pnpm verify:final-goal-audit` is `blocked` with
  `implementationFixableCount=17`, `policyBlockedCount=3`, and
...
| `pnpm verify:goal:safe` | ordered final safe gate; currently includes a consented token-spending prompt and must not be treated as no-token |
