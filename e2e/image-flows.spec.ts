 ...
 * 2. A real PNG piped through `aelyris-imgcat.ps1` → real PTY → ConPTY →
 ...
 *   shipped `scripts/aelyris-imgcat.{ps1,sh}` emitters + PNG fixtures.
...
test.describe("Inline image round-trip via Tauri CDP", () => {
   ...
   * `aelyris-imgcat.ps1` against a fixture. `-NoProfile` keeps the prompt
  ...
  test("PNG piped via aelyris-imgcat surfaces an ImageRef and round-trips bytes back", async () => {
    const tid = await spawnTerminal();
    try {
