...
async function main() {
  ...
  const setRes = await call("aelyris.context.set", { key, value: "jwt" });
  ...
  const got = await call("aelyris.context.get", { key });
  ...
  const resolved = await call("aelyris.intent.resolve", { id: intentId, status: "accepted" });
  ok(resolved.intent?.status === "accepted", "intent.resolve converges the proposal");
  const openAfter = await call("aelyris.intent.list");
  ...
  const recent3 = await call("aelyris.event.recent");
