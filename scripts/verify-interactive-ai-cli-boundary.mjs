...
function startSidecar(port, tempRoot) {
  const child = spawn(SIDECAR, [], {
    ...
    env: {
      ...
      AETHER_MUX_SNAPSHOT_DIR: join(tempRoot, "mux"),
      AETHER_PTY_SCROLLBACK_DIR: join(tempRoot, "scrollback"),
    },
    shell: false,
...
function writeShimFiles(binDir) {
  ...
  writeFileSync(
    ...
    [
      ...
      'const cli = process.argv[2] || process.env.AETHER_AGENT_CLI || "unknown";',
      'const marker = process.env.AETHER_CLI_BOUNDARY_MARKER || "NO_MARKER";',
      'const model = process.env.AETHER_AGENT_MODEL || "unknown";',
  ...
  const created = await request(base, "/commands", {
    ...
    body: JSON.stringify({
      ...
      env: {
        ...
        AETHER_AGENT_CLI: cli,
        AETHER_AGENT_MODEL: `${cli}-boundary-smoke`,
