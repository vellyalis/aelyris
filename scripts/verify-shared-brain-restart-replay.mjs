...
//
// Usage:
//   AELYRIS_API_TOKEN=... node scripts/verify-shared-brain-restart-replay.mjs --phase seed
//   # restart Aelyris
//   AELYRIS_API_TOKEN=... node scripts/verify-shared-brain-restart-replay.mjs --phase verify --id <printed-id>
//
// It verifies that Context Store, durable event log, file ownership, symbol
// ownership, and aelyris.shared_brain.snapshot survive the process boundary.
...
const id = arg("id", process.env.AELYRIS_RESTART_REPLAY_ID);

if (!TOKEN) {
  const report = {
    schema: "aelyris.shared-brain-restart-replay/v1",
    phase,
    ok: false,
    ...
    blockers: [
      {
        capability: "aelyris-api-token",
        message: "AELYRIS_API_TOKEN is required for the two-phase live restart replay verifier",
        phase: "host-preflight",
        command: "node scripts/verify-shared-brain-restart-replay.mjs",
      },
    ],
    nextAction:
      "Run the seed phase against a live authenticated Aelyris API, restart Aelyris, then run the verify phase with the printed id.",
  };
...
async function seed() {
  ...
  await call("aelyris.context.set", { key: seed.decisionKey, value: seed.decisionValue });
  await call("aelyris.ownership.assign", { agentId: seed.agentId, pattern: seed.filePattern });
  await call("aelyris.symbol.claim", {
  ...
  await call("aelyris.agent.report_blocker", {
    sessionId: seed.agentId,
    summary: `restart replay marker ${runId}`,
    needs: "verify after restart",
  });
  const snapshot = await call("aelyris.shared_brain.snapshot", { workspaceId: `restart-${runId}` });
  const failures = [];
  ...
  writeFileSync(
    OUT,
    `${JSON.stringify({ schema: "aelyris.shared-brain-restart-replay/v1", phase: "seed", ok: true, seed }, null, 2)}\n`,
  );
  console.log(`\nSeeded restart replay id: ${runId}`);
  console.log(
    `Restart Aelyris, then run: node scripts/verify-shared-brain-restart-replay.mjs --phase verify --id ${runId}`,
  );
}
...
async function verify() {
  ...
  const symbols = await call("aelyris.symbol.claims");
  ok(
  ...
  const events = await call("aelyris.event.since", { afterSeq: 0, limit: 1000 });
  ok(
  ...
  const snapshot = await call("aelyris.shared_brain.snapshot", { workspaceId: `restart-${seed.id}` });
  ok(
