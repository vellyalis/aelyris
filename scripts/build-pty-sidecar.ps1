...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "cargo build aetherctl failed with exit code $LASTEXITCODE"
  }

  $Extension = if ($IsWindowsPlatform) { ".exe" } else { "" }
  $Built = Join-Path $TauriDir "pty-server/target/release/aether-pty-server$Extension"
  $Bundled = Join-Path $TauriDir "binaries/aether-pty-server-$HostTriple$Extension"
  $BuiltCtl = Join-Path $TauriDir "target/release/aetherctl$Extension"
  $AccidentalMainPackageBin = Join-Path $TauriDir "target/release/aether-pty-server$Extension"
  ...
  Write-Host "Prepared aetherctl: $BuiltCtl"
}
finally {
  Pop-Location
}
