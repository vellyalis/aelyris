...
- Extend the existing `aelyris.event.ack` adapter, `EventBus::ack`, `EventRepo`,
  ...
  actor field, delivery protocol, identity store, or AI-only event path.
- Keep caller-supplied consumer id, event id, and sequence as delivery inputs. They
  identify the at-least-once cursor operation but do not define the initiating actor.
- Preserve exact event-id/sequence validation, monotonic cumulative acknowledgement,
  mismatch/gap/corruption typed errors, idempotent retry behavior, and the existing
  durable Event Bus as the sole cursor authority. `event.poll` and `event.since` remain
  read projections rather than a second acknowledgement owner.
- Retain accepted/rejected audit with actor, operation, acknowledged sequence/outcome,
  and stable one-way consumer/event/input digests. Do not persist raw consumer ids,
  event ids, Event Bus payloads, bearer values, environment values, or repository
  contents in the audit record.
- A failed acknowledgement must remain explicit and must not be replayed merely to
  obtain audit evidence. Audit-write failure must not create a second cursor result.
...
- Extend the existing `aelyris.request_approval` adapter, WatchdogEngine,
  ...
  signal, Governance, and durable audit journal. Do not add an approval engine, queue,
  policy store, actor field, identity store, or AI-only decision path.
- Keep caller-supplied session id, requested tool, optional summary, and risk as request
  inputs. They describe the approval request but do not define the initiating actor.
- Preserve watchdog auto-approve/auto-deny rules, pending-user outcome, bounded queue
  capacity, oldest-item eviction, overflow observability, typed errors, and the existing
  queue/watchdog owners. This slice does not add a grant tool or weaken AIO-22's separate
  human approval capability.
- Retain accepted/rejected audit with actor, operation, decision class, queue insertion
  and overflow outcome when known, and stable one-way session/tool/input digests. Do not
  persist raw session ids, tool names, summaries, risks, rule names, pending-item ids,
  approval content, bearer values, environment values, or repository contents.
- Queue, overflow-publication, or audit-write failure must stay explicit without
  replaying the request, duplicating a pending item, or fabricating a second watchdog
  decision.
...
- Extend the existing `aelyris.proofbook.settle_agent_session` and
  ...
  already required, Governance, and durable audit/ledger owners. Do not add another
  runner, proof store, cancellation owner, actor field, identity store, or AI-only path.
- Keep caller-supplied project, run, step, and completion proof values as Proofbook data.
  They never define the initiating actor. Preserve the safer runtime-owned settlement
  and exact-revision cancellation tools as the preferred product paths.
- Preserve current proof validation, ledger state-machine checks, startup admission,
  cancellation semantics, typed errors, and existing durable runner as the sole
  mutation authority. Do not weaken proof requirements or claim process termination.
- Retain accepted/rejected audit with actor, operation, resulting ledger revision/status
  when known, and stable one-way run/input digests. Do not persist raw project paths,
  run/step ids, proof bodies, artifact paths, done signals, reviewer ids, summaries,
  blocker contents, bearer values, environment values, or repository contents.
- Failure or audit-write failure must not replay a settlement/cancellation, fabricate a
...
### AIO-34 — MCP Unified Fleet Coverage
...
- Extend the existing `aelyris.fleet_status` adapter over the existing `AgentManager`,
  ...
  AIO-33 safe projection. Do not add a fleet manager, session registry, synchronization
  owner, polling loop, or second lifecycle model.
- Merge current headless and interactive snapshots without mutating either owner. Keep
  exact session ids and run modes, preserve each owner's current status/usage values,
  and return deterministic ordering. Duplicate ids must fail closed or be represented
  once by an explicit deterministic rule rather than silently producing ambiguity.
- Preserve AIO-33 exclusions for prompts, approval text, cwd/workspace/repository/
  worktree paths, branch names, full lineage, and recycle failure contents. Do not
  convert unavailable owners into invented sessions or known telemetry.
- Expose `headlessAvailable`, `interactiveAvailable`, per-runtime counts, and one
  combined `sessions` projection. Reading fleet status must not reap, stop, resume,
...
- Done: MCP HTTP alone submitted one isolated Goal and bounded context to the backend
  default real-Codex planner. Mission `019fdfd7-ea93-7983-ae56-d23cb03aa98a`, plan
  `019fdfd7-ea93-7983-ae56-d24652fd2d3c`, and task `add-aio45-test` were atomically
  accepted. The value-minimized response exposed only Mission/plan identity, bounded
  task coordination fields, and explicit false caller-authority flags; it exposed no
  raw Goal, context, repository path, descriptions, output paths, branch names, model
  assignments, or symbol values. Target main remained
  `b6132dff126931688eba69100344fba94860d94d`, the repository stayed clean with one
  worktree, and the same Mission/plan/task identity and `ready` status restored from
  the same SQLite authority without a second planner invocation.

### AIO-46 — MCP Value-Minimized Mission Continuity
...
- Add `aelyris.mission.current` over the existing `current_cockpit_mission` and
  ...
  plan, task, status, packet, branch, model, or query-shaping identity.
- Return the exact Mission/plan ids and revisions, accepted status, bounded task ids and
  statuses, task/status counts, and restart-safe coordination flags. Do not return the
  raw Goal/request, repository path, planner prompt/response, task descriptions,
  dependency values, output paths, branch names, model assignments, symbol values, or
  packet contents.
- Preserve one read owner and one SQLite authority. Do not cache a second current-
  Mission projection in MCP and do not infer continuity from volatile Event Bus cache.
- Use a structured `notFound` outcome when the repository has no accepted cockpit
  Mission; absence is not an empty synthetic Mission.
...
- Done: `aelyris.mission.replay` accepts only `repoPath` and delegates exactly once to
  `mission_replay::replay_current_mission` over the attached TaskManager and durable
  Event Bus owners. It creates no replay state, cursor, checkpoint, recovery, Git,
  terminal, provider, review, merge, settlement, or Event ACK effect.
- Done: the response contains exact Mission/plan identity and revision, one 64-character
  replay hash, bounded task/execution/Mission-event/packet counts, durable event
  frontier, and explicit zero-effect/second-owner guarantees. Raw Goal/context,
  repository path, Task/execution identity or payload, Event payload, OID, review/
  evidence, packet identity, and packet contents remain absent.
- Done: Principal-bound audit stores only actor, one-way repository/input/replay-hash
  digests, outcome, rejection code, aggregate counts, and false exposure flags. Two
  repeated reads returned the same raw replay hash while audit retained only a distinct
  domain-separated digest; TaskGraph, Event frontier, and target Git HEAD stayed exact.
- Done: focused CLI-only tests passed `8 / 8`, including schema authority, not-found,
  exposure, real MCP dispatch, repeated-read stability, audit minimization, canonical
  replay, finite bounds, unrelated-event isolation, and same-SQLite restart equality.

### V2-M2 — Historical Replay Reducer Baseline
...
- Seed only from the immutable accepted Mission task plan, then reduce the exact
  ...
  second history table, TaskGraph, checkpoint store, event mirror, or replay cache.
- Produce bounded checkpoint projections containing only relative event position,
  event kind, task-status counts, completed-work count, packet-backed Mission state,
  and a canonical checkpoint hash. Raw payloads, paths, commands, provider output,
  OIDs, review/evidence, and packet contents remain outside the projection.
- Fail closed on an illegal lifecycle transition, unknown referenced task, duplicate or
  regressing event identity, missing completion packets, event gap/corruption, or finite
  bound overflow. Never silently skip a Mission-scoped event the reducer claims to own.
- The final reconstructed checkpoint must agree with the current TaskGraph statuses and
