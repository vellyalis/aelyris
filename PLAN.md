...
audit. The aim is to make Astra more than a terminal and more than a message
CLI: a local-first workspace where agents, panes, tasks, reviews, worktrees, and
evidence stay connected.

## Current Truth

Astra/Aether is still alpha and not release-ready. Current local evidence shows
...
Astra should become the calm cockpit for multi-agent development.
...
- Add Astra-specific policies: `review_gate`, `task_scope`, `human_only`.
...
- Define `aether.directive.v1` schema while Astra remains the public-name candidate.
...
- Public docs may then say Astra is an agmsg-class local messaging superset.
