...
fn main() {
    ...
    // its Win11 acrylic translucency; set AELYRIS_ENABLE_WEBVIEW2_CDP=1 only when you need a CDP
    // endpoint for E2E/inspection (and accept the opaque window for that run).
    if cfg!(debug_assertions) && std::env::var("AELYRIS_ENABLE_WEBVIEW2_CDP").as_deref() == Ok("1") {
