...
fn build_tools_list_value() -> serde_json::Value {
    let mut catalog = serde_json::json!({
        ...
        "tools": [
            ...
            },
            {
                ...
                "description": "Create an isolated agent worktree as the authenticated Principal through the existing Git owner. The accepted response returns the path-free worktree identity/status projection plus a repository digest; repository and absolute worktree paths are not echoed. Durable audit remains target-minimized.",
                "safety": "FREE",
                "inputSchema": {
                    "type": "object",
                    "required": ["repoPath", "branchName"],
            ...
            },
            {
                ...
                "description": "Remove the branch/name returned by worktree create/list and optionally delete its branch as the authenticated Principal. The accepted response keeps exact worktree name and deletion intent plus a repository digest, but does not echo repository or absolute worktree paths. Durable audit remains target-minimized.",
                "safety": "FREE",
                "inputSchema": {
                    "type": "object",
            ...
            },
            {
                ...
                "description": "Read one prompt-free, path-free projection across headless and visible interactive Aelyris agents. Exact session identity, run mode, and coordination status remain available, while prompts, approval text, and machine-local repository/worktree paths are excluded.",
                "safety": "FREE",
                "inputSchema": { "type": "object", "additionalProperties": false }
            },
            {
    ...
    tools.push(serde_json::json!({
        ...
        "description": "Read the restart-safe immutable completion receipt for the exact current accepted cockpit Mission. The authenticated caller supplies only repoPath. The existing SQLite-backed TaskManager and settlement packets remain the sole owners; this read never invokes a planner, reviewer, merge, settlement replay, Event ACK, or Git mutation. Returns not_found, pending, blocked, or completed. Completed requires packet-backed Done for every declared task, one exact WorkPacket reference per task, the aggregate MissionCompletionPacket, and a stable receipt digest. It never returns raw Goal/context, repository/worktree paths, task identities/descriptions, branches/models/symbols, OIDs, event history, review/evidence, or packet contents.",
        "safety": "FREE",
        "accessMode": "observe-only",
        "outputSensitivity": "value-minimized-coordination",
        "readOnly": true,
    ...
    tools.push(serde_json::json!({
        ...
        "description": "Read a bounded newest checkpoint window from the deterministic V2 Mission replay timeline for the exact current accepted cockpit Mission. The authenticated caller supplies only repoPath plus an optional server-clamped limit. The backend fully reduces and validates the existing immutable plan, TaskGraph, Mission-scoped durable events, executions, CompletedWorkPackets, and MissionCompletionPacket before truncating only the returned checkpoint list. Returns exact Mission/plan identity, timeline hash, total/returned counts, hasMore, safe checkpoint summaries/hashes, final status/completion summary, source aggregates, and zero-effect guarantees. It exposes no Goal/context, repository path, task/execution identity or payload, event id/payload/global sequence, OID, review/evidence, packet identity/contents, checkpoint private material, or recovery/rollback authority.",
        "safety": "FREE",
        "accessMode": "observe-only",
        "outputSensitivity": "value-minimized-coordination",
        "readOnly": true,
    ...
    tools.push(serde_json::json!({
        ...
        "description": "Trigger the existing backend-owned Mission review, exact-OID merge, and immutable settlement path only for an exact task in Review within the current accepted cockpit Mission. Before reviewer, candidate-freeze, or merge effects, the backend requires exact current-Mission membership, TaskGraph Review status, and matching durable Mission/plan/WorkUnit activation lineage; generic, stale, unrelated, mixed-graph, and non-Review tasks fail closed. The authenticated Principal supplies only repository and task identity and cannot supply a verdict, reviewer identity, candidate/target OID, merge token, gate result, WorkPacket, or MissionCompletionPacket. Every successful merge requires an immutable task WorkPacket; MissionCompletionPacket is returned when aggregate Mission completion is due. The fixed independent reviewer and existing Task/Review/Merge/settlement owners remain authoritative. Authority audit stores only one-way repository/task/input digests, current-Mission preflight outcome, and result flags—never paths, OIDs, review findings, or packet ids.",
        "safety": "GATED",
        "inputSchema": {
            "type": "object",
