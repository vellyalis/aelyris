...
const POWERSHELL_STARTUP_SCRIPT: &str = r#"try { Set-PSReadLineOption -PredictionSource None } catch {}; if ($env:AELYRIS_SHELL_INTEGRATION -ne '1') { $env:AELYRIS_SHELL_INTEGRATION = '1'; $script:__aelyris_esc = [char]27; $script:__aelyris_bel = [char]7; $script:__aelyris_original_prompt = (Get-Item function:prompt).ScriptBlock; function script:__aelyris_commandEnd([int]$exit) { "$($script:__aelyris_esc)]133;D;$exit$($script:__aelyris_bel)" }; function global:prompt { $success = $?; $lastExit = if ($success) { 0 } elseif ($LASTEXITCODE) { $LASTEXITCODE } else { 1 }; $body = & $script:__aelyris_original_prompt; "$(script:__aelyris_commandEnd $lastExit)$($script:__aelyris_esc)]133;A$($script:__aelyris_bel)$body$($script:__aelyris_esc)]133;B$($script:__aelyris_bel)" } }"#;
...
mod tests {
    ...
    fn powershell_startup_args_skip_profile_and_keep_prediction_guard() {
        ...
        assert!(args
            .iter()
            .any(|arg| arg.contains("AELYRIS_SHELL_INTEGRATION")));
