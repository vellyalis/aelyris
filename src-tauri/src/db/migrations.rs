...
pub fn run_migrations(conn: &Connection) -> Result<(), rusqlite::Error> {
    conn.execute_batch(
        ...
        -- The Knowledge Graph (code dependency map agents reason over). Persisted
        -- as a whole-graph snapshot (atomically replaced) so the populated graph
        -- survives an app restart instead of resetting to empty. `node_json` holds
        -- the full serde CodeNode (forward-compatible via #[serde(default)]).
