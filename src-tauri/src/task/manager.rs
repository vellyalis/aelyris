...
use super::mission::{
    ...
    CockpitTaskSettlementOutcome, CompletedWorkPacket, MissionCompletionPacket,
    ...
    SettlementBlockerKind, SettlementNextAction, SettlementNextActionKind,
    A7_SETTLEMENT_PROOF_VERSION, BLOCKED_WORK_PACKET_SCHEMA, COCKPIT_GOVERNANCE_POLICY_ID,
...
impl TaskManager {
    ...
    pub fn attach_db(&self, db: Arc<ManagedDb>) -> Result<usize, String> {
        ...
        // Preserve only Mission attempts whose exact durable facts prove that a
        // backend owner can resume them. Review/EffectStarted without a review
        // record remains uncertain and is deliberately not admitted; an exact
        // cockpit merge receipt may preserve Review only long enough for the
        // packet owner to settle it.
        let mut resumable_mission_reviews = HashSet::new();
        for attempt in &loaded_executions {
            let Some(task) = loaded.get(&attempt.identity.task_id) else {
                continue;
            };
            let resumable = db.with(|database| {
                Ok::<_, String>(
                    Self::a7_acceptance_resume_fact(database, attempt)?
                        || Self::cockpit_settlement_resume_fact(database, attempt, task.status)?
                        || Self::cockpit_merge_reconciliation_candidate_fact(
                            database,
                            attempt,
                            task.status,
                        )?,
    ...
    fn cockpit_settlement_resume_fact(
        db: &crate::db::Database,
        attempt: &WorkExecutionAttempt,
    ...
    fn cockpit_merge_reconciliation_candidate_fact(
        db: &crate::db::Database,
        attempt: &WorkExecutionAttempt,
        task_status: TaskStatus,
    ) -> Result<bool, String> {
    ...
    pub fn submit_cockpit_plan(
        ...
        let activations = super::mission::cockpit_activations_from_plan(
            &preview,
    ...
    pub fn persist_mission_gate_evidence(
        ...
        self.validate_mission_gate_evidence_binding(activation, evidence)?;
        let db = self.db().ok_or(MissionPlanError::DurabilityUnavailable)?;
        let _writer = self.persistence_lock();
        db.try_with(|database| TaskRepo::insert_mission_gate_evidence(database, evidence))
    }

    fn validate_mission_gate_evidence_binding(
        &self,
        activation: &MissionPlanActivation,
        evidence: &MissionGateEvidence,
        ...
        let cockpit_suite = evidence.cockpit_gate_suite()?;
        let a7_binding = cockpit_suite.is_none()
            && evidence.base_oid == activation.accepted_base_oid
            && evidence.command_argv == activation.test_argv;
        let cockpit_binding = cockpit_suite.is_some()
            && activation.test_argv == [super::mission::COCKPIT_GATE_SUITE_CONTRACT_VERSION]
            && cockpit_candidate_binding_is_current(activation, evidence)?;
        if attempt.runtime != super::ExecutionRuntime::VisiblePty
            || attempt.identity.attempt_id != evidence.attempt_id
            || attempt.identity.execution_generation != evidence.execution_generation
            || attempt.identity.agent_run_id != evidence.agent_run_id
    ...
    pub fn persist_mission_review_bundle(
        &self,
        activation: &MissionPlanActivation,
        evidence: &MissionGateEvidence,
        invocation: &crate::review::ReviewerInvocation,
        review: &crate::review::MissionReviewRecord,
    ) -> Result<crate::review::MissionReviewRecord, MissionPlanError> {
        self.validate_mission_gate_evidence_binding(activation, evidence)?;
        crate::review::mission::validate_mission_review_record(review)
        ...
        if review.activation_id != activation.activation_id
            ...
            || review.tested_evidence_id != evidence.evidence_id
            || review.reviewed_oid != evidence.tested_oid
            || review.reviewer_invocation_receipt_ref != invocation.receipt().receipt_ref()
        {
            return Err(MissionPlanError::Validation(
                "Mission review bundle does not share one immutable activation/evidence lineage"
                    .into(),
            ));
        }
        let db = self.db().ok_or(MissionPlanError::DurabilityUnavailable)?;
        ...
        db.try_with(|database| {
            database
                ...
                .map_err(|error| {
                    MissionPlanError::Persistence(format!("begin Mission review bundle: {error}"))
                })?;
            let result = (|| {
                TaskRepo::insert_mission_gate_evidence(database, evidence)?;
                crate::persistence::ReviewRepo::insert_reviewer_invocation_receipt(
                    database,
                    invocation.receipt(),
                )
                .map_err(MissionPlanError::Persistence)?;
                crate::persistence::ReviewRepo::insert_mission_review(database, review)
                    .map_err(MissionPlanError::Persistence)?;
                Ok::<_, MissionPlanError>(review.clone())
            })();
            match result {
                Ok(record) => {
                    database.conn().execute_batch("COMMIT").map_err(|error| {
                        MissionPlanError::Persistence(format!(
                            "commit Mission review bundle: {error}"
                        ))
                    })?;
                    Ok(record)
                }
                Err(error) => {
                    let _ = database.conn().execute_batch("ROLLBACK");
                    Err(error)
                }
            }
        })
...
fn project_cockpit_settlement_completion(
    graph: &mut TaskGraph,
    task_id: &str,
...
fn cockpit_candidate_binding_is_current(
    activation: &MissionPlanActivation,
    evidence: &MissionGateEvidence,
    ...
    let target_oid =
        crate::git::resolve_branch_oid(&activation.repository_root, &activation.target_branch)
            .map_err(MissionPlanError::Persistence)?;
    let source_oid =
        crate::git::resolve_branch_oid(&activation.repository_root, &activation.source_branch)
            .map_err(MissionPlanError::Persistence)?;
    if target_oid != evidence.base_oid || source_oid != evidence.tested_oid {
        return Ok(false);
    }
    let repository = git2::Repository::open(&activation.repository_root)
        .map_err(|error| MissionPlanError::Persistence(error.to_string()))?;
    ...
    let candidate = evidence
        ...
        .parse::<git2::Oid>()
        .map_err(|error| MissionPlanError::Validation(error.to_string()))?;
    let target_descends_from_plan = target == accepted
        || repository
            .graph_descendant_of(target, accepted)
            .map_err(|error| MissionPlanError::Persistence(error.to_string()))?;
    let candidate_descends_from_target = candidate != target
        && repository
            .graph_descendant_of(candidate, target)
            .map_err(|error| MissionPlanError::Persistence(error.to_string()))?;
    Ok(target_descends_from_plan && candidate_descends_from_target)
}
...
fn observe_settlement_git(
    ...
    observe_settlement_git_from_base(activation, &activation.accepted_base_oid, tested_oid)
}

fn observe_cockpit_settlement_git(
    activation: &MissionPlanActivation,
    evidence: &MissionGateEvidence,
) -> SettlementGitObservation {
...
fn observe_settlement_git_from_base(
    activation: &MissionPlanActivation,
    base_oid: &str,
    tested_oid: Option<&str>,
) -> SettlementGitObservation {
...
fn cockpit_settlement_coverage(
    preview: &MissionPlanPreview,
    activation: &MissionPlanActivation,
    evidence: &MissionGateEvidence,
    review: &crate::review::MissionReviewRecord,
...
mod tests {
    ...
    fn cockpit_goal_plan_and_active_graph_restore_from_one_durable_transaction() {
        ...
        assert_eq!(accepted.work_units.len(), 2);
        assert_eq!(accepted.mission_definition.acceptance.len(), 2);
        assert_eq!(
            accepted
                .cockpit_task_plan
                .as_ref()
        ...
        let task_work_units = accepted
            ...
            .map(|task| {
                (
                    ...
                    task.work_unit_id
                        .clone()
                        .expect("settlement cockpit Task has a WorkUnit identity"),
                )
            })
            .collect::<std::collections::BTreeMap<_, _>>();
        let activations = first
            .mission_activations(&accepted.plan_id, accepted.plan_revision)
            .unwrap();
        assert_eq!(activations.len(), 2);
        assert_eq!(
            activations
                .iter()
                .map(|activation| { (activation.task_id.clone(), activation.work_unit_id.clone()) })
                .collect::<std::collections::BTreeMap<_, _>>(),
            task_work_units
        );
    ...
    #[test]
    fn cockpit_settlement_accepts_owned_rework_history_beyond_one_direct_child() {
        let directory = tempfile::tempdir().unwrap();
        let repository = git2::Repository::init(directory.path()).unwrap();
        repository.set_head("refs/heads/main").unwrap();
        std::fs::create_dir_all(directory.path().join("src")).unwrap();
        std::fs::write(directory.path().join("src/lib.rs"), "pub fn base() {}\n").unwrap();
        let mut index = repository.index().unwrap();
        index.add_path(std::path::Path::new("src/lib.rs")).unwrap();
        index.write().unwrap();
        let tree_id = index.write_tree().unwrap();
        let tree = repository.find_tree(tree_id).unwrap();
        let signature = git2::Signature::now("Cockpit rework", "rework@example.invalid").unwrap();
        ...
        let repo_path = directory.path().to_string_lossy().into_owned();
        let source_branch = "agent/cockpit-rework";
        crate::control::worktree::ensure_for_branch(&repo_path, source_branch).unwrap();
        ...
        .unwrap();
        let first_oid = crate::control::worktree::commit_owned_for_branch(
            &repo_path,
            source_branch,
            &["src/lib.rs".into()],
        ...
        .unwrap();
        let corrected_oid = crate::control::worktree::commit_owned_for_branch(
            &repo_path,
            source_branch,
            &["src/lib.rs".into()],
            "correct reviewed findings",
        )
        .unwrap()
        .unwrap();
        assert_ne!(first_oid, corrected_oid);
        assert!(matches!(
            crate::git::perform_merge_bound(
                &repo_path,
                source_branch,
                "main",
                &corrected_oid,
            ...
            .unwrap(),
            crate::git::BoundMergeResult::Merged(crate::git::MergeOutcome::FastForwarded { .. })
        ));

        let activation = MissionPlanActivation {
            schema: "aelyris.mission_plan_activation/v1".into(),
            activation_id: uuid::Uuid::now_v7().to_string(),
            plan_id: uuid::Uuid::now_v7().to_string(),
            plan_revision: 1,
            mission_id: uuid::Uuid::now_v7().to_string(),
            mission_revision: 1,
            work_unit_id: uuid::Uuid::now_v7().to_string(),
            task_id: "cockpit-rework".into(),
            plan_content_digest: "1".repeat(64),
            accepted_base_oid: base_oid.clone(),
            repository_root: repo_path,
            source_branch: source_branch.into(),
            target_branch: "main".into(),
            owned_targets: vec!["src/lib.rs".into()],
            test_argv: vec![super::super::mission::COCKPIT_GATE_SUITE_CONTRACT_VERSION.into()],
            activated_by: uuid::Uuid::now_v7().to_string(),
        ...
        let evidence = MissionGateEvidence {
            ...
            agent_run_id: uuid::Uuid::now_v7().to_string(),
            runtime_domain_id: "visible_pty".into(),
            pty_session_id: uuid::Uuid::now_v7().to_string(),
            gate_id: super::super::mission::COCKPIT_GATE_SUITE_ID.into(),
            contract_version: super::super::mission::COCKPIT_GATE_SUITE_CONTRACT_VERSION.into(),
            command_argv: Vec::new(),
            ...
            base_oid,
            candidate_oid: corrected_oid.clone(),
            tested_oid: corrected_oid.clone(),
            started_at_unix_ms: 1,
            ended_at_unix_ms: 2,
        };

        let observation = observe_cockpit_settlement_git(&activation, &evidence);
        assert_eq!(observation.candidate_state, "exact-owned-clean");
        assert_eq!(
            observation.candidate_oid.as_deref(),
            Some(corrected_oid.as_str())
        );
        assert_eq!(
            observation.target_oid.as_deref(),
            Some(corrected_oid.as_str())
        );
        assert_eq!(observation.changed_paths, ["src/lib.rs"]);
        assert!(observation.owned_diff_digest.is_some());
    }

    #[test]
    fn cockpit_settlement_completion_atomically_readies_dependents() {
        let mut graph = TaskGraph::new();
        graph.add(Task::new("root", "Root")).unwrap();
        graph
            .add(Task::new("child", "Child").with_dependencies(["root".to_string()]))
            .unwrap();
        graph.recompute_ready();
        graph.transition("root", TaskStatus::Running).unwrap();
        graph.transition("root", TaskStatus::Review).unwrap();

        let changed = project_cockpit_settlement_completion(&mut graph, "root").unwrap();

        assert_eq!(graph.get("root").unwrap().status, TaskStatus::Done);
        assert_eq!(graph.get("child").unwrap().status, TaskStatus::Ready);
        assert!(changed.iter().any(|task_id| task_id == "child"));
    }
